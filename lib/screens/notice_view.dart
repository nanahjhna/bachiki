...
class _NoticeViewState extends State<NoticeView> with SingleTickerProviderStateMixin { late final TabController _tab = TabController(length: 2, vsync: this); bool received = false; @override void dispose() { _tab.dispose(); super.dispose(); }
  @override Widget build(BuildContext context) => Scaffold(appBar: const GameHeader(titleKey: 'notice'), body: Column(children: [TabBar(controller: _tab, tabs: const [Tab(text: '공지사항'), Tab(text: '출석 이벤트')]), Expanded(child: TabBarView(controller: _tab, children: [_notices(), _attendance()]))]), bottomNavigationBar: const GameBottomNavigation());
